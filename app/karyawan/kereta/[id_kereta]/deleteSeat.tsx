  ...
  const handleSubmit = async (e: FormEvent) => {
      try {
        ...
        if (response.data.success == true) {
          toast(message, {
            containerId: `toastDropSeat-${myProp.item.id}`,
            type: "success",
          });
    ...
    return (
        <div>
          <ToastContainer containerId={`toastDropSeat-${myProp.item.id}`} />
          <button
            type="button"
            onClick={() => openModal()}
            className="size-5 flex items-center justify-center text-white rounded-md shadow-md"
          >
            <svg
          ...
          <Modal isShow={show}>
            ...
            <form onSubmit={(e) => handleSubmit(e)}>
              <div className="w-full p-3 rounded-t-lg">
                ...
                <h1 className="font-bold text-lg">Delete Seat</h1>
                <span className="text-sm text-slate-400">
                  Pastikan menghapus dengan benar
                </span>
              </div>
