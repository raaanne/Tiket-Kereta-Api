...
const EditAdmin = (myProp: props) => {
  ...
  const openModal = () => {
    ...
    setAddres(myProp.admin.address)
  ...
  const handleSubmit = async (e: FormEvent) => {
        try {
          ...
          const requestData = { name, username, nik, address, phone };
          ...
          if (response.data.success == true) {
            toast(message, { containerId: `toastEditAdmin-${myProp.admin.id}`, type: "success" });
            setShow(false);
            // reload page
            setTimeout(() => router.refresh(), 1000);
          } else {
            toast(message, { containerId: `toastEditAdmin-${myProp.admin.id}`, type: "warning" });
          }
        } catch (error) {
          console.log(error);
